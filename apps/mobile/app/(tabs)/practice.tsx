...
function TypingIndicator() {
  return (
    <View className="flex-row items-end gap-2 px-4 py-2">
      ...
      <View className="rounded-2xl rounded-bl-sm border border-slate-700 bg-slate-800 px-4 py-3">
        <View className="flex-row items-center gap-2">
          <ActivityIndicator size="small" color="#06B6D4" />
          <Text className="text-muted-foreground text-sm">Typing...</Text>
        </View>
      </View>
    </View>
  );
...
function MessageBubble({
  ...
  return (
    <View className={`flex-row items-end gap-2 px-4 py-1 ${isUser ? 'flex-row-reverse' : ''}`}>
      {!isUser && (
        <View className="h-8 w-8 shrink-0 items-center justify-center rounded-full border border-slate-700 bg-slate-800">
          <Icon as={BotIcon} className="size-4 text-cyan-400" />
        </View>
      )}
      ...
      <View style={{ maxWidth: '78%' }} className="gap-1">
          ...
          <Text className={`text-sm leading-5 ${isUser ? 'text-white' : 'text-slate-100'}`}>
            {segments.map((seg, i) =>
              seg.type === 'text' ? (
                ...
                <Text key={i} className="font-semibold text-amber-300">
                  {seg.content}
                </Text>
              )
            )}
        ...
        {corrections.length > 0 &&
          corrections.map((seg, i) => (
            <View
              key={i}
              className="gap-1 rounded-xl border border-amber-500/20 bg-amber-950/30 px-3 py-2.5">
              ...
              <View className="mt-1 flex-row items-center gap-3">
                ...
                <TouchableOpacity onPress={() => handleAddVocab(seg.content)} activeOpacity={0.7}>
      ...
      <View className="bg-background flex-1">
        ...
        <View className="border-border flex-row items-center justify-between border-b px-4 py-4">
          ...
          <TouchableOpacity
            ...
            <Icon as={XIcon} className="text-muted-foreground size-6" />
          </TouchableOpacity>
        </View>
        ...
        <ScrollView className="flex-1 px-4 py-6" keyboardShouldPersistTaps="handled">
          ...
          <View className="mb-6 gap-2">
            <Text className="text-foreground text-sm font-medium">Topic (optional)</Text>
            <TextInput
              value={topic}
          ...
          <View className="gap-3">
            <Text className="text-foreground text-sm font-medium">Suggestions</Text>
              ...
              {TOPIC_SUGGESTIONS.map((s) => (
                <TouchableOpacity
                  ...
                  className={`rounded-full border px-3 py-2 ${
                    topic === s ? 'border-cyan-600 bg-cyan-600' : 'bg-card border-border'
                  }`}>
        ...
        <View className="border-border border-t px-4 py-4">
          <TouchableOpacity
            ...
            disabled={isLoading}
            activeOpacity={0.7}
            className="items-center rounded-xl bg-cyan-600 py-4">
            {isLoading ? (
              <ActivityIndicator color="white" />
            ) : (
              <Text className="text-base font-semibold text-white">Start Conversation</Text>
            )}
          </TouchableOpacity>
        </View>
      </View>
...
export default function PracticeScreen() {
  ...
  const renderConversationItem = ({ item }: { item: ConversationSummary }) => (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
      className="border-border flex-row items-center gap-3 border-b px-4 py-4">
      <View className="h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-700 bg-slate-800">
      ...
      <View className="min-w-0 flex-1 gap-1">
        <View className="flex-row items-center justify-between gap-2">
          ...
          <Text
            className={`text-xs font-semibold ${CEFR_COLORS[item.cefrLevel] ?? 'text-muted-foreground'}`}>
            {item.cefrLevel}
          </Text>
        </View>
  ...
  if (view === 'list') {
    return (
      <View className="bg-background flex-1">
        {/* Header */}
        <View className="border-border flex-row items-center justify-between border-b px-4 pb-3 pt-4">
          <View>
            <Text className="text-foreground text-xl font-bold">Practice</Text>
            <Text className="text-muted-foreground text-sm">AI Conversation</Text>
          </View>
          <TouchableOpacity
            onPress={() => setShowNewModal(true)}
            activeOpacity={0.7}
            className="flex-row items-center gap-2 rounded-xl bg-cyan-600 px-4 py-2">
        ...
        {error && (
          <Animated.View
            entering={FadeIn}
            exiting={FadeOut}
            className="mx-4 mt-3 rounded-xl border border-red-800 bg-red-950 px-4 py-3">
        ...
        {isLoadingList ? (
          ...
          <View className="flex-1 items-center justify-center gap-3">
            <ActivityIndicator size="large" color="#06B6D4" />
            <Text className="text-muted-foreground text-sm">Loading conversation...</Text>
          </View>
        ) : conversations.length === 0 ? (
          <View className="flex-1 items-center justify-center gap-5 px-8">
            <View className="h-16 w-16 items-center justify-center rounded-2xl border border-slate-700 bg-slate-800">
            ...
            <TouchableOpacity
              onPress={() => setShowNewModal(true)}
              activeOpacity={0.7}
              className="rounded-xl bg-cyan-600 px-8 py-3">
              <Text className="font-semibold text-white">New Conversation</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <FlatList
    ...
    <KeyboardAvoidingView
      ...
      <View className="border-border flex-row items-center gap-3 border-b px-4 py-3">
        ...
        <View className="min-w-0 flex-1">
          ...
          <Text className="text-muted-foreground text-xs">
            ...
            {activeConversation?.cefrLevel ? (
              <Text
                className={` ${CEFR_COLORS[activeConversation.cefrLevel] ?? 'text-muted-foreground'}`}>
                {' '}
                • {activeConversation.cefrLevel}
              </Text>
            ) : null}
        ...
        {activeConversation && (
          <TouchableOpacity
            ...
            hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
            className="p-1">
            <Icon as={TrashIcon} className="text-muted-foreground size-5" />
          </TouchableOpacity>
        )}
      </View>
      ...
      {error && (
        <Animated.View
          entering={FadeIn}
          exiting={FadeOut}
          className="mx-4 mt-2 rounded-xl border border-red-800 bg-red-950 px-4 py-3">
      ...
      {/* Input area */}
      <View className="border-border flex-row items-end gap-2 border-t px-4 py-3">
        <TextInput
          ...
          multiline
          maxLength={1000}
          className="bg-card border-border flex-1 rounded-2xl border px-4 py-3"
