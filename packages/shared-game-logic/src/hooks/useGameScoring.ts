...
import { calculateTimeBonus, calculateAccuracyMultiplier, calculateFinalScore } from '../utils/scoring'
...
export function useGameScoring(): GameScoringState & GameScoringActions {
  ...
  const computeScore = useCallback(
    (basePoints: number, elapsedMs: number, timeLimitMs?: number): GameScore => {
      const timeBonus =
        timeLimitMs !== undefined ? calculateTimeBonus(elapsedMs, timeLimitMs) : 0
